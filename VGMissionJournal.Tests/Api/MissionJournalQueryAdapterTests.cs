...
public class MissionJournalQueryAdapterTests
{
    private static (MissionStore store, MissionJournalQueryAdapter adapter) Build()
    {
    ...
    {
        var (_, adapter) = Build();
        Assert.Equal(JournalSchema.CurrentVersion, adapter.SchemaVersion);
    }
    ...
    {
        var adapter = new MissionJournalQueryAdapter(SampleStore());
    ...
    [Fact]
    public void MissionJournalApi_Current_StartsNull()
    {
        ...
        // handle.
        Assert.Null(MissionJournalApi.Current);
    }
}
