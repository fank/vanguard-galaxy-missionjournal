...
/// VGMissionJournal type by direct name in the assertions below (only in
/// setup to seed the store) — if a consumer mod does the same, it can
/// interop without a hard dependency on VGMissionJournal.dll.
///
...
/// </summary>
[Collection("MissionJournalApi.Current")]
public class ReflectionProbeTests : IDisposable
{
    ...
    public ReflectionProbeTests()
    {
        ...
        MissionJournalApi.Current = new MissionJournalQueryAdapter(_store);
    }

    public void Dispose() => MissionJournalApi.Current = null;
    ...
    public void FacadeType_ResolvesViaAssemblyQualifiedName()
    {
        ...
        Assert.True(facadeType!.IsAbstract && facadeType.IsSealed,
            "MissionJournalApi should be a static class (abstract + sealed).");
    }
    ...
    public void SchemaVersion_ReadableViaReflection_IsInt()
    {
        ...
        Assert.Equal(JournalSchema.CurrentVersion, value);
    }
    ...
    public void GetMissionsInSystem_ReturnsListOfMissionRecord_ViaReflection()
    {
        ...
        // Iterate without casting to any VGMissionJournal type — properties
    ...
    private static object? GetCurrentViaReflection() =>
        Type.GetType("VGMissionJournal.Api.MissionJournalApi, VGMissionJournal")!
            .GetProperty("Current")!
            .GetValue(null);
}
